...
foreach ($flags as $flag) {
    ...
    } elseif ($key === 'trading-floor-stories') {
        $trading_floor_flags[] = $flag;
...
:root{color-scheme:dark}*{box-sizing:border-box}body{margin:0;background:#0e0e0e;color:#f1f1f1;font:15px/1.5 Inter,system-ui,sans-serif}.shell{max-width:1180px;margin:0 auto;padding:32px 24px 56px}.eyebrow{color:#f2ca50;font-size:11px;font-weight:800;letter-spacing:.14em;text-transform:uppercase}.head{display:flex;justify-content:space-between;align-items:end;gap:20px;margin-bottom:24px}.head h1{margin:8px 0 0;font-size:34px;letter-spacing:-.03em}.head p{margin:8px 0 0;color:#8b9098}.back{display:inline-flex;align-items:center;min-height:44px;padding:0 14px;border:1px solid #242424;border-radius:10px;color:#c9d8ff;text-decoration:none;background:#151515}.flash{margin-bottom:18px;padding:12px 16px;border:1px solid rgba(242,202,80,.28);border-radius:10px;background:rgba(242,202,80,.09);color:#ffe082}.admin-nav{position:sticky;top:12px;z-index:5;display:flex;gap:8px;flex-wrap:wrap;margin:0 0 20px;padding:8px;border:1px solid #242424;border-radius:14px;background:#151515}.admin-nav a{padding:10px 14px;border-radius:9px;color:#8b9098;text-decoration:none}.admin-nav a:hover,.admin-nav a:focus-visible,.admin-nav a.active{background:rgba(242,202,80,.12);color:#ffe082}.grid{display:grid;grid-template-columns:1fr;gap:18px;align-items:start}.panel{border:1px solid #242424;border-radius:16px;background:#151515;padding:20px}.panel h2{margin:0;font-size:19px}.panel-intro{margin:5px 0 16px;color:#858c98;font-size:13px}.list{display:grid;gap:12px}.row{padding:16px;border:1px solid #242424;border-radius:14px;background:#111;scroll-margin-top:92px}.row:target{border-color:rgba(242,202,80,.55);box-shadow:0 0 0 3px rgba(242,202,80,.08)}.row-top{display:flex;justify-content:space-between;gap:16px;align-items:start}.label{font-weight:750}.meta{margin-top:3px;color:#777d86;font-size:13px}.status{display:flex;align-items:center;gap:12px;color:#8b9098;font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.06em}.switch{position:relative;width:48px;height:28px}.switch input{opacity:0;width:0;height:0}.slider{position:absolute;inset:0;border-radius:999px;background:#303030;cursor:pointer;transition:.2s}.slider:before{content:'';position:absolute;width:20px;height:20px;left:4px;top:4px;border-radius:50%;background:#888;transition:.2s}.switch input:checked+.slider{background:#b18b24}.switch input:checked+.slider:before{transform:translateX(20px);background:#fff3b1}.roles{display:flex;flex-wrap:wrap;gap:8px;margin-top:12px}.role-pill{display:inline-flex;align-items:center;gap:6px;padding:6px 10px;border:1px solid #2c2c2c;border-radius:999px;background:#171717;color:#babec5;font-size:12px}.role-pill input{accent-color:#f2ca50}.actions{display:flex;justify-content:space-between;align-items:center;gap:12px;margin-top:14px}.small-btn,.danger-btn{border:1px solid #2c2c2c;border-radius:10px;background:#191919;color:#f1f1f1;padding:9px 12px;cursor:pointer}.danger-btn{color:#ffb8b8;border-color:#4a2a2a;background:#1b1212}.debug-panel{display:none}.section-anchor{scroll-margin-top:92px}@media(max-width:900px){.head{align-items:start;flex-direction:column}}@media(max-width:600px){.shell{padding-inline:14px}.row-top,.actions{flex-direction:column;align-items:start}.admin-nav{top:6px}.admin-nav a{flex:1 1 calc(50% - 8px);text-align:center}}
.admin-nav button{border:0;padding:10px 14px;border-radius:9px;background:transparent;color:#8b9098;font:inherit;cursor:pointer}.admin-nav button:hover,.admin-nav button:focus-visible,.admin-nav button.active{background:rgba(242,202,80,.12);color:#ffe082}.admin-pane{display:none}.admin-pane.active{display:block}</style></head><body><main class="shell"><div class="head"><div><div class="eyebrow">2RICH INTERNAL</div><h1>Admin Control Panel</h1><p>Manage app features and limit visibility by role.</p></div><a class="back" href="/dashboard/">Back to dashboard</a></div><?php if ($flash): ?><div class="flash"><?php echo esc_html($flash); ?></div><?php endif; ?><div class="debug-panel"><?php echo htmlspecialchars(json_encode(['table'=>$table,'resolved_table'=>$resolved_table,'flags'=>$flags,'runtime'=>['__FILE__'=>__FILE__,'wp_load_path'=>dirname(__DIR__, 2) . '/wp-load.php','db_name'=>defined('DB_NAME') ? DB_NAME : null,'db_host'=>defined('DB_HOST') ? DB_HOST : null,'table_count'=>isset($wpdb) ? $wpdb->get_var("SELECT COUNT(*) FROM " . $resolved_table) : null]], JSON_PRETTY_PRINT)); ?></div><nav class="admin-nav" aria-label="Admin sections" role="tablist"><button type="button" class="active" data-admin-tab="core-features">Core features</button><button type="button" data-admin-tab="dashboard-cards">Dashboard cards</button><button type="button" data-admin-tab="trading-floor">Trading Floor</button></nav><div class="grid"><section class="panel section-anchor admin-pane" id="trading-floor"><h2>Trading Floor</h2><p class="panel-intro">Control the Trading Floor stories row and its temporary-unavailable overlay.</p><div class="list"><?php foreach ($trading_floor_flags as $flag): $is_on = (int)($flag['is_enabled'] ?? 0) === 1; $overlay_on = (int)($flag['is_overlay_enabled'] ?? 0) === 1; ?><div class="row"><div class="row-top"><div><div class="label"><?php echo esc_html($flag['label']); ?></div><div class="meta"><?php echo esc_html($flag['description']); ?></div></div><form method="post" class="status"><input type="hidden" name="admin_nonce" value="<?php echo esc_attr(wp_create_nonce('rich_admin_features')); ?>"><input type="hidden" name="feature_action" value="toggle"><input type="hidden" name="flag_key" value="<?php echo esc_attr($flag['flag_key']); ?>"><input type="hidden" name="is_enabled" value="0"><label class="switch"><input type="checkbox" name="is_enabled" value="1" <?php checked($is_on); ?> onchange="this.form.submit()"><span class="slider"></span></label><span><?php echo $is_on ? 'Enabled' : 'Disabled'; ?></span></form></div><form method="post"><input type="hidden" name="admin_nonce" value="<?php echo esc_attr(wp_create_nonce('rich_admin_features')); ?>"><input type="hidden" name="feature_action" value="overlay"><input type="hidden" name="flag_key" value="<?php echo esc_attr($flag['flag_key']); ?>"><label class="status"><span>Temporary overlay</span><label class="switch"><input type="checkbox" name="is_overlay_enabled" value="1" <?php checked($overlay_on); ?> onchange="this.form.submit()"><span class="slider"></span></label><span><?php echo $overlay_on ? 'Visible' : 'Off'; ?></span></label><div class="actions"><input type="text" name="overlay_message" value="<?php echo esc_attr($flag['overlay_message'] ?? 'This feature is temporarily unavailable.'); ?>" style="flex:1;min-height:38px;padding:0 10px;border:1px solid #2c2c2c;border-radius:10px;background:#111;color:#f1f1f1"><button class="small-btn" type="submit">Save overlay</button></div></form></div><?php endforeach; ?></div></section><section class="panel section-anchor admin-pane active" id="core-features"><h2>Core features</h2><p class="panel-intro">Control access to the main areas of the app.</p><div class="list"><?php foreach ($core_feature_flags as $flag): $selected_roles = array_filter(array_map('trim', explode(',', (string)($flag['allowed_roles'] ?? '')))); $is_on = (int)($flag['is_enabled'] ?? 0) === 1; ?><div class="row"><div class="row-top"><div><div class="label"><?php echo esc_html($flag['label']); ?></div><div class="meta"><code><?php echo esc_html($flag['flag_key']); ?></code><?php if (!empty($flag['description'])): ?> · <?php echo esc_html($flag['description']); ?><?php endif; ?></div></div><form class="status" method="post"><span><?php echo $is_on ? 'On' : 'Off'; ?></span><input type="hidden" name="feature_action" value="toggle"><input type="hidden" name="flag_key" value="<?php echo esc_attr($flag['flag_key']); ?>"><input type="hidden" name="admin_nonce" value="<?php echo esc_attr(wp_create_nonce('rich_admin_features')); ?>"><label class="switch"><input type="checkbox" name="is_enabled" value="1" <?php checked($is_on,true); ?> onchange="this.form.submit()"><span class="slider"></span></label></form></div><form method="post"><input type="hidden" name="feature_action" value="roles"><input type="hidden" name="flag_key" value="<?php echo esc_attr($flag['flag_key']); ?>"><input type="hidden" name="admin_nonce" value="<?php echo esc_attr(wp_create_nonce('rich_admin_features')); ?>"><div class="roles"><?php foreach ($roles as $role_key => $role_name): ?><label class="role-pill"><input type="checkbox" name="allowed_roles[]" value="<?php echo esc_attr($role_key); ?>" <?php checked(in_array($role_key, $selected_roles, true)); ?>><?php echo esc_html($role_name); ?></label><?php endforeach; ?></div><div class="actions"><button class="small-btn" type="submit">Save role visibility</button><?php if (!in_array($flag['flag_key'], $core_flags, true)): ?><button class="danger-btn" type="submit" name="feature_action" value="delete" onclick="return confirm('Remove this feature flag?');">Remove feature</button><?php else: ?><span class="meta">Core feature</span><?php endif; ?></div></form><form method="post"><input type="hidden" name="feature_action" value="overlay"><input type="hidden" name="flag_key" value="<?php echo esc_attr($flag['flag_key']); ?>"><input type="hidden" name="admin_nonce" value="<?php echo esc_attr(wp_create_nonce('rich_admin_features')); ?>"><div style="margin-top:12px;padding-top:12px;border-top:1px solid #242424;"><p style="margin:0 0 10px;font-size:13px;font-weight:700;">Overlay Message</p><div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;"><label class="switch"><input type="checkbox" name="is_overlay_enabled" value="1" <?php checked((int)($flag['is_overlay_enabled'] ?? 0) === 1, true); ?> onchange="this.form.submit()"><span class="slider"></span></label><span style="font-size:13px;">Show temporarily unavailable overlay</span></div><?php if ((int)($flag['is_overlay_enabled'] ?? 0) === 1): ?><div style="margin-top:10px;"><input type="text" name="overlay_message" value="<?php echo esc_attr($flag['overlay_message'] ?? 'This feature is temporarily unavailable.'); ?>" style="width:100%;padding:8px 10px;border:1px solid #2c2c2c;border-radius:8px;background:#111;color:#f1f1f1;font-size:13px;" placeholder="Overlay message"></div><div class="actions"><button class="small-btn" type="submit">Save overlay</button></div><?php endif; ?></div></form></div><?php endforeach; ?></div></section><section class="panel section-anchor admin-pane" id="dashboard-cards"><h2>Dashboard cards</h2><p class="panel-intro">Manage dashboard cards, visibility, and temporary overlays.</p><div class="list"><?php if (!$dashboard_card_flags): ?><div class="row"><div class="label">No dashboard card flags found</div><div class="meta">The database currently has no flags beginning with <code>card-</code>.</div></div><?php else: foreach ($dashboard_card_flags as $flag): $is_on=(int)($flag['is_enabled']??0)===1; ?><div class="row"><div class="row-top"><div><div class="label"><?php echo esc_html($flag['label']); ?></div><div class="meta"><code><?php echo esc_html($flag['flag_key']); ?></code><?php if (!empty($flag['description'])): ?> · <?php echo esc_html($flag['description']); ?><?php endif; ?></div></div><form class="status" method="post"><span><?php echo $is_on?'On':'Off'; ?></span><input type="hidden" name="feature_action" value="toggle"><input type="hidden" name="flag_key" value="<?php echo esc_attr($flag['flag_key']); ?>"><input type="hidden" name="admin_nonce" value="<?php echo esc_attr(wp_create_nonce('rich_admin_features')); ?>"><label class="switch"><input type="checkbox" name="is_enabled" value="1" <?php checked($is_on,true); ?> onchange="this.form.submit()"><span class="slider"></span></label></form></div><form method="post"><input type="hidden" name="feature_action" value="overlay"><input type="hidden" name="flag_key" value="<?php echo esc_attr($flag['flag_key']); ?>"><input type="hidden" name="admin_nonce" value="<?php echo esc_attr(wp_create_nonce('rich_admin_features')); ?>"><div class="roles"><label class="role-pill"><input type="checkbox" name="is_overlay_enabled" value="1" <?php checked((int)($flag['is_overlay_enabled']??0),1); ?>> Temporary overlay</label><button class="small-btn" type="submit" style="margin-left:8px;">Save overlay</button></div><input name="overlay_message" value="<?php echo esc_attr($flag['overlay_message']??'This feature is temporarily unavailable.'); ?>" style="width:100%;margin-top:10px;padding:9px;border:1px solid #2c2c2c;border-radius:9px;background:#171717;color:#f1f1f1"></form></div><?php endforeach; endif; ?></div></section></div></main><script>document.querySelectorAll('[data-admin-tab]').forEach(function(button){button.addEventListener('click',function(){var id=button.getAttribute('data-admin-tab');document.querySelectorAll('[data-admin-tab]').forEach(function(b){b.classList.toggle('active',b===button);b.setAttribute('aria-selected',b===button?'true':'false')});document.querySelectorAll('.admin-pane').forEach(function(p){p.classList.toggle('active',p.id===id)});history.replaceState(null,'','#'+id)})});(function(){var id=location.hash.slice(1);var b=document.querySelector('[data-admin-tab="'+id+'"]');if(b)b.click()})();</script></body></html>
