...
if($post_id<=0 || !get_post($post_id)){http_response_code(400);echo json_encode(['success'=>false,'message'=>'Invalid post']);exit;}
...
$comment_rows=$wpdb->get_results($wpdb->prepare("SELECT c.id,c.body,c.created_at,c.user_id,u.display_name FROM {$comments} c LEFT JOIN {$wpdb->users} u ON u.ID=c.user_id WHERE c.post_id=%d ORDER BY c.created_at ASC",$post_id),ARRAY_A);
echo json_encode(['success'=>true,'is_liked'=>(bool)$wpdb->get_var($wpdb->prepare("SELECT id FROM {$likes} WHERE post_id=%d AND user_id=%d",$post_id,$user_id)),'is_saved'=>(bool)$wpdb->get_var($wpdb->prepare("SELECT id FROM {$saves} WHERE post_id=%d AND user_id=%d",$post_id,$user_id)),'likes_count'=>(int)$wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM {$likes} WHERE post_id=%d",$post_id)),'comments_count'=>count($comment_rows),'shares_count'=>(int)$wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM {$shares} WHERE post_id=%d",$post_id)),'comments'=>$comment_rows]);
