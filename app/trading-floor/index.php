...
<head>
    ...
    <style>
        ...
        .group-feed-text-link { display: inline-block; transition: opacity .18s ease, transform .18s ease; }
        .group-feed-text-link:hover { opacity: .88; transform: translateY(-1px); }
...
<!-- Middle Feed -->
    ...
    function renderFeedPostModal(post) {
        ...
        const authorProfileHref = `?user_id=${encodeURIComponent(Number(post.user_id || 0))}`;
        const authorLinkStart = Number(post.user_id || 0) > 0 ? `<a href="${authorProfileHref}" class="group-feed-text-link" style="color:inherit;text-decoration:none;">` : '';
        const authorLinkEnd = Number(post.user_id || 0) > 0 ? '</a>' : '';
