...
<head>
    ...
    <style>
        ...
        .group-feed-top { display: flex; align-items: flex-start; justify-content: space-between; gap: 12px; margin-inline: 12px; }
        .group-feed-author { display:flex; align-items:center; gap:12px; min-width:0; }
        .group-feed-author-copy { min-width:0; display:flex; flex-direction:column; }
        .group-feed-author-link { display:flex; align-items:center; gap:12px; min-width:0; color:inherit; text-decoration:none; border-radius:10px; transition:opacity .18s ease, transform .18s ease; }
        .group-feed-author-link:hover { opacity:.88; transform:translateY(-1px); }
        .group-feed-author-link:focus-visible { outline:2px solid #F2CA50; outline-offset:4px; }
        ...
        .group-feed-action.liked { color:#f87171; }
        .group-feed-action.liked svg { fill:#f87171; }
        .group-feed-action.bookmarked { color:#F2CA50; }
        .group-feed-action-spacer { flex:1; }
        ...
        .post-trade-card.social-trade-variant { margin:12px 12px 0; width:calc(100% - 24px); border-radius:10px; }
        .trade-card-header {
        ...
        @media (max-width: 900px) { .feed-post-modal { grid-template-columns:1fr; width:min(96vw, 720px); height:min(86vh, 760px); } .feed-post-media { min-height:280px; max-height:52vh; height:52vh; } }
    ...
    <div class="dashboard-container">
        ...
        <aside class="sidebar">
            <ul class="sidebar-menu">
                ...
                <li class="menu-item active" <?php echo !rich_feature_enabled('trading-floor', true, $user_id) ? 'style="opacity: 0.5;"' : ''; ?> onclick="window.location.href='/trading-floor'">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
<!-- Middle Feed -->
    ...
    function normalizeSignalGroup(group) {
        return {
            ...
            active_call_link: group.active_call_link || '',
    ...
    function renderGroupsPanel() {
        ...
        const current = groups.find(g => String(g.id) === String(floorSignalsState.activeGroupId))
            ...
            || (!floorSignalsState.activeGroupId && joinedGroups[0] ? normalizeSignalGroup({
                ...
                active_call_link: joinedGroups[0].active_call_link || '',
    ...
    function getPostMediaUrls(post) {
        const urls = [];
        const candidates = Array.isArray(post && post.image_urls) ? post.image_urls : [];
        candidates.forEach(item => {
            const url = typeof item === 'string' ? item : (item && (item.url || item.image_url));
    ...
    function movePostCarousel(button, step) {
        const media = button.closest('.social-post-media');
        if (!media) return;
        const slides = Array.from(media.querySelectorAll('.social-post-media-slide'));
        if (!slides.length) return;
    ...
    function renderSocialPostCard(post, compact = false) {
        ...
        const authorProfileHref = `?user_id=${encodeURIComponent(Number(post.user_id || 0))}`;
        const authorLinkStart = Number(post.user_id || 0) > 0 ? `<a class="group-feed-author-link" href="${authorProfileHref}" style="color:inherit;text-decoration:none;" onclick="event.stopPropagation();">` : '';
        ...
        const groupBadge = groupName ? `<span style="font-size:13px; font-weight:400; opacity:0.8; color:#888; margin-left:6px; margin-right:4px;">at</span><a style="font-size:13px; font-weight:700; color:#F2CA50; letter-spacing:0.02em; text-transform:none; text-decoration:none; cursor:pointer;" onclick="event.stopPropagation(); openFloorSignalGroup(${post.group_id})">${groupName}</a>` : '';
        let content = '';
        if (layout === 'trade_card') {
            ...
            content = `<div class="social-layout-analysis">${symbol ? `<div class="social-layout-symbol">${symbol}${direction ? ` · ${direction}` : ''}</div>` : ''}${caption ? `<div class="group-feed-body">${caption.replace(/\n/g, '<br>')}</div>` : ''}${tagsMarkup}${media}</div>`;
        ...
        const authorTextLinkStart = Number(post.user_id || 0) > 0 ? `<a href="${authorProfileHref}" style="color:inherit;text-decoration:none;" onmouseover="this.style.textDecoration='underline'" onmouseout="this.style.textDecoration='none'" onclick="event.stopPropagation();">` : '';
    ...
    let tfFeedModalIndex = -1;
        ...
        host.innerHTML = posts.map((post, index) => {
            ...
            const mediaUrl = escapeHtml(mediaUrls[0] || '');
            const isVideo = /\.(mp4|webm|mov)(\?.*)?$/i.test(mediaUrls[0] || '');
            const carouselBadge = mediaUrls.length > 1 ? `<span class="post-meta-badge">${mediaUrls.length} slides</span>` : '';
            if (mediaUrl && isVideo) {
                return `<button type="button" class="profile-post-thumb" data-post-id="${escapeHtml(post.id)}" data-feed-index="${index}" onclick="openFeedPostModalByIndex(${index})">${carouselBadge}<span class="post-meta-badge">${typeLabel}</span><video src="${mediaUrl}" style="width:100%;height:100%;object-fit:cover;" muted loop playsinline></video></button>`;
            }
            if (mediaUrl) {
    ...
    function renderFeedPostModal(post) {
        ...
        if (avatar) {
            ...
            if (Number(post.user_id || 0) > 0) {
                avatar.style.cursor = 'pointer';
                avatar.onclick = () => window.location.href = `?user_id=${encodeURIComponent(Number(post.user_id || 0))}`;
            } else {
                avatar.style.cursor = 'default';
                avatar.onclick = null;
            }
        }
        const authorName = escapeHtml(post.author_name || 'Trader');
        const groupName = post.group_name ? escapeHtml(post.group_name) : '';
        const groupBadge = groupName ? `<span style="font-size:13px; font-weight:400; opacity:0.8; color:#888; margin-left:6px; margin-right:4px;">at</span><a style="font-size:13px; font-weight:700; color:#F2CA50; letter-spacing:0.02em; text-transform:none; text-decoration:none; cursor:pointer;" onclick="closeFeedPostModal(); openFloorSignalGroup(${post.group_id})">${groupName}</a>` : '';
        const authorProfileHref = `?user_id=${encodeURIComponent(Number(post.user_id || 0))}`;
        const authorLinkStart = Number(post.user_id || 0) > 0 ? `<a href="${authorProfileHref}" style="color:inherit;text-decoration:none;">` : '';
        const authorLinkEnd = Number(post.user_id || 0) > 0 ? '</a>' : '';
        title.innerHTML = `<span style="font-weight:500;">${authorLinkStart}${authorName}${authorLinkEnd}</span>${groupBadge}`;
    ...
    let feedTouchEndX = 0;
    const feedPostMedia = document.getElementById('feedPostModalMedia');
    if (feedPostMedia) {
        feedPostMedia.addEventListener('touchstart', (event) => {
            feedTouchStartX = event.changedTouches[0].clientX;
        }, { passive: true });
        feedPostMedia.addEventListener('touchend', (event) => {
            feedTouchEndX = event.changedTouches[0].clientX;
            const delta = feedTouchEndX - feedTouchStartX;
            if (Math.abs(delta) < 40) return;
            if (delta < 0) showNextFeedPost(1);
            else showNextFeedPost(-1);
        }, { passive: true });
    }
    ...
    document.addEventListener('keydown', (event) => {
        ...
        const createModalOpen = !!(createModal && createModal.classList.contains('active'));
        if (event.key === 'Escape') {
            if (feedModalOpen) {
                event.preventDefault();
                closeFeedPostModal();
